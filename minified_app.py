_A8='Loqin - Waiting for Wi-Fi selection'
_A7='Status: Choose Wi-Fi to continue'
_A6='update_checker'
_A5='perf_thread'
_A4='Loqin - Active'
_A3='Check for Updates'
_A2='Performance Mode'
_A1='Loqin • Live Network Monitor'
_A0='Missing credentials'
_z='OFFLINE'
_y='Referer'
_x='Grand Total'
...
_s='Upload'
_r='eyebrow'
_q='color: #66c7ff; font-size: 14px;'
_p='wifi.svg'
_o='Unknown'
_n='%I:%M:%S %p'
_m='color: #ff7b88; font-size: 14px;'
_l=b'opacity'
_k='skeletonTile'
_j='cardsContainer'
_i='refresh'
_h='Refresh'
_g='Optimizing Network...'
_f='Resume Loqin'
_e='Pause Loqin'
_d='Show Speed Graph'
_c='ONLINE'
_b='#8ff3c8'
_a='color: #a7b0d6; font-size: 14px;'
_Z='secured'
_Y='ignore'
_X='utf-8'
_W='interfaces'
...
def is_auto_start_enabled():
	...
	if sys.platform!=_O:return _B
	try:A=winreg.OpenKey(winreg.HKEY_CURRENT_USER,REG_PATH,0,winreg.KEY_READ);winreg.QueryValueEx(A,APP_REG_NAME);winreg.CloseKey(A);return _A
	except OSError:return _B
	except Exception as B:print(f"Failed to read registry: {B}");return _B
	...
	def nativeEventFilter(A,eventType,message):
		...
		if B.message==WM_POWERBROADCAST:
			...
			if B.wParam==PBT_APMSUSPEND:
				...
				A.tray_app.has_checked_for_updates=_B;QTimer.singleShot(2000,A.tray_app.auto_connect_last_wifi)
				if hasattr(A.tray_app,'perf_action')and A.tray_app.perf_action.isChecked():QTimer.singleShot(5000,lambda:A.tray_app.trigger_performance_mode(checked=_A))
		return _B,0
class PerformanceModeThread(QThread):
	status_signal=pyqtSignal(str,str)
	def __init__(A,use_best=_A):super().__init__();A.use_best=use_best
	def run(A):
		J='Performance Mode ON';K=_g if A.use_best else'Reverting Network...';A.status_signal.emit(K,_D)
		try:
			...
			F=_C
			try:
				O=subprocess.check_output([_R,_S,_V,_W],creationflags=134217728).decode(_X,errors=_Y)
				for G in O.split('\n'):
					if _P in G and _P==G.split(_E)[0].strip():
						I=G.split(_E)
...
def get_current_wifi_ssid():
	...
	if sys.platform!=_O:return''
	try:
		C=subprocess.check_output([_R,_S,_V,_W],creationflags=134217728,timeout=5).decode(_X,errors=_Y)
		for D in C.splitlines():
			A=D.strip()
			if A.startswith('SSID')and not A.startswith(_P):
				B=A.split(_E,1)
...
class WiFiConnectThread(QThread):
	...
	def run(A):
		...
		try:
			B=subprocess.run([_R,_S,'connect',f"name={A.ssid}"],capture_output=_A,text=_A,creationflags=134217728,timeout=10);C=time.time()+12
			while time.time()<C:
	...
	def run(B):
		try:
			...
			for D in F.scan_results():
				...
				if H is _C or G>H[_N]:C[A]={_I:A,_N:G,_Z:D.akm!=[const.AKM_TYPE_NONE]}
...
class _LegacyWiFiPickerDialog(QDialog):
	...
	def show_skeleton_loading(B,count=6,columns=2):
		...
		for E in range(count):C=QFrame();C.setObjectName(_k);F=QGraphicsOpacityEffect(C);C.setGraphicsEffect(F);A=QPropertyAnimation(F,_l);A.setDuration(1200);A.setStartValue(.25);A.setKeyValueAt(.5,.75);A.setEndValue(.25);A.setEasingCurve(QEasingCurve.Type.InOutSine);A.setLoopCount(-1);A.start();B.skeleton_anims.append(A);G=E//D;H=E%D;B.cards_layout.addWidget(C,G,H)
	...
	def scan_networks(A):
		...
		if not A.initial_scan_done:A.status.setText(_T)
		A.scan_thread.start()
	def on_scan_failed(A,error):A.status.setText(f"Scan failed: {error}");A.status.setStyleSheet(_m)
	def on_scan_finished(A,networks):
		C=networks;D=[];E=[];C.sort(key=lambda x:x[_N],reverse=_A)
		for B in C:
			if not B.get(_Z,_A):D.append(B)
			else:E.append(B)
		A.update_networks_ui(D,E);F=datetime.now().strftime(_n);A.status.setText(f"Scan complete. Last updated at {F}");A.status.setStyleSheet(_a)
	def update_networks_ui(B,portal_networks,normal_networks,columns=2):
		F=columns;E=[(A,_A)for A in portal_networks]+[(A,_B)for A in normal_networks]
		...
		for C in range(max(len(E),len(B.reusable_network_buttons))):
			if C<len(E):
				D,G=E[C];H=D.get(_I,_o)if isinstance(D,dict)else str(D);O=int(D.get(_N,-100))if isinstance(D,dict)else-100;I=wifi_signal_color(O)
				if C>=len(B.reusable_network_buttons):A=QPushButton();A.setCursor(Qt.CursorShape.PointingHandCursor);A.setMinimumHeight(80);B.reusable_network_buttons.append(A);P=C//F;Q=C%F;B.cards_layout.addWidget(A,P,Q)
				A=B.reusable_network_buttons[C];A.setText(f"  {H}");A.setIcon(QIcon(resource_path(_p)));A.setIconSize(QSize(32,32))
				if G:J='qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #66c7ff, stop:1 #bb7cff)';K='qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #7ad0ff, stop:1 #c78eff)';L='qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #50b9f0, stop:1 #a86be6)';M='#06101c';N=f"1px solid rgba(255, 255, 255, 0.3); border-left: 5px solid {I}"
				else:J='rgba(16, 21, 38, 0.74)';K='rgba(28, 36, 64, 0.9)';L='rgba(12, 15, 30, 0.9)';M='#f4f7fb';N=f"1px solid rgba(146, 160, 215, 0.18); border-left: 5px solid {I}"
				A.setStyleSheet(f"""
                    QPushButton {{
                        color: {M};
                        background: {J};
                    ...
                    QPushButton:hover {{
                        background: {K};
                        border-color: {"rgba(255, 255, 255, 0.5)"if G else"rgba(102, 199, 255, 0.4)"};
                    }}
                    QPushButton:pressed {{
                        background: {L};
                    }}
				...
				A.clicked.connect(lambda checked=_B,target_ssid=H:B.connect_to_network(target_ssid));A.setVisible(_A)
			else:B.reusable_network_buttons[C].setVisible(_B)
	def connect_to_network(A,ssid):A.is_connecting=_A;A.status.setText(f"Connecting to {ssid}...");A.status.setStyleSheet(_q);A.auto_refresh_timer.stop();A.wifi_chosen.emit(ssid);A.accept()
class WiFiPickerDialog(QDialog):
	...
	def scan_networks(A):
		...
		A.scan_thread.start()
	def on_scan_failed(A,error):A.status.setText(f"Scan failed: {error}");A.status.setStyleSheet(_m)
	def on_scan_finished(A,networks):B=networks;B.sort(key=lambda item:item.get(_N,-100),reverse=_A);C=[B for B in B if A.portal_pattern.fullmatch(B.get(_I,'').strip())];D=[A for A in B if A not in C];A.update_networks_ui(C,D);A.status.setText(f"{len(B)} networks found • updated {datetime.now().strftime(_n)}");A.status.setStyleSheet(_a)
	def update_networks_ui(A,portal_networks,normal_networks,columns=2):
		G=normal_networks;F=portal_networks;B=columns;A.skeleton_anims.clear();A.clear_cards_layout();A.initial_scan_done=_A;D=[]
		if F:D.append(('PORTAL WI-FI',F,_A))
		if G:D.append(('OTHER NETWORKS',G,_B))
		C=0
		for(J,H,K)in D:
			E=QLabel(J);E.setObjectName(_r);E.setStyleSheet('color: #a7b0d6; font-size: 11px; font-weight: 800; letter-spacing: 2px; padding: 8px 2px 0;');A.cards_layout.addWidget(E,C,0,1,B);C+=1
			for(I,L)in enumerate(H):M=I%B;N=C+I//B;A.add_network_card(L,K,N,M)
			C+=(len(H)+B-1)//B
	def add_network_card(C,network,is_portal,row,column):B=network;D=B.get(_I,_o);E=int(B.get(_N,-100));F='Open network'if not B.get(_Z,_A)else'Secured network';A=QPushButton(f"{D}\n{F}  •  {E} dBm");A.setObjectName('portalCard'if is_portal else'wifiCard');A.setIcon(QIcon(resource_path(_p)));A.setIconSize(QSize(30,30));A.setMinimumHeight(94);A.setCursor(Qt.CursorShape.PointingHandCursor);A.clicked.connect(lambda checked=_B,target=D:C.connect_to_network(target));C.cards_layout.addWidget(A,row,column)
	def connect_to_network(A,ssid):A.is_connecting=_A;A.status.setText(f"Connecting to {ssid}…");A.status.setStyleSheet(_q);A.auto_refresh_timer.stop();A.wifi_chosen.emit(ssid);A.accept()
...
class AccountDetailsDialog(QDialog):
	...
	def populate_table(A,rows_data,grand_total_data):
		...
		if F:
			C=A.table.rowCount();A.table.insertRow(C);E=QTableWidgetItem(_x);E.setForeground(QColor(_b));E.setTextAlignment(Qt.AlignmentFlag.AlignCenter);A.table.setItem(C,0,E)
			for(J,D)in enumerate(F):B=QTableWidgetItem(D);B.setForeground(QColor(_b));B.setTextAlignment(Qt.AlignmentFlag.AlignCenter);A.table.setItem(C,J+3,B)
...
class ConfigManager:
	...
	@staticmethod
	def load_config():
		A={_K:'',_U:10,'auto_connect':_A,_Q:''};ConfigManager.ensure_dir_exists();B=_B
		if os.path.exists(CONFIG_FILE):
			try:
				with open(CONFIG_FILE,'r')as E:
					...
					if _L in C:
						D=C[_L]
						if D and A[_K]:ConfigManager.set_password(A[_K],D)
						if _L in A:del A[_L]
						B=_A
			except Exception:pass
		else:B=_A
...
class NetworkWorker(QThread):
	...
	def check_network_state(B):
		try:
			...
			if A.status_code==204:return _c
			else:return'PORTAL'
		except requests.exceptions.RequestException:return _z
	def run(A):
		while A.is_running:
			if A.is_paused:A.sleep(1);continue
			B=A.config.get(_K);C=ConfigManager.get_password(B)
			if not B or not C:A.status_signal.emit(_A0,_M);return
			D=A.check_network_state()
			if D==_c:A.status_signal.emit('Connected',_G);return
			elif D==_z:A.status_signal.emit('Waiting for Wi-Fi...',_D);return
	...
	def login(A,username,password):
		...
		try:
			...
			else:A.status_signal.emit('Login failed. Check credentials.',_M)
		except Exception as K:print(K);A.status_signal.emit('Portal timeout or error.',_M)
class SpeedGraphDialog(QDialog):
	def __init__(A,parent=_C):I='#59658a';F='bottom';E='left';D='#a7b0d6';super().__init__(parent);A.setWindowTitle(_A1);A.setWindowIcon(QIcon(resource_path(_J)));A.resize(820,520);A.download_history=[0]*60;A.upload_history=[0]*60;C=QVBoxLayout(A);C.setContentsMargins(15,15,15,15);B=QHBoxLayout();G=QLabel('Real-Time Network Usage');G.setStyleSheet("color: #f4f7fb; font-family: 'Space Grotesk', 'Segoe UI', sans-serif; font-size: 20px; font-weight: 700;");A.pin_checkbox=QCheckBox('Always on Top');A.pin_checkbox.setCursor(Qt.CursorShape.PointingHandCursor);A.pin_checkbox.setStyleSheet('\n            QCheckBox {\n                color: #a7b0d6;\n                font-size: 13px;\n                font-weight: 500;\n            }\n            QCheckBox::indicator {\n                width: 14px;\n                height: 14px;\n                border: 1px solid rgba(146,160,215,0.4);\n                border-radius: 5px;\n                background: rgba(255,255,255,0.04);\n            }\n            QCheckBox::indicator:checked {\n                background: #66c7ff;\n                border: 1px solid #66c7ff;\n            }\n            QCheckBox:hover {\n                color: #f4f7fb;\n            }\n        ');A.pin_checkbox.toggled.connect(A.toggle_always_on_top);A.secret_code='nyan';A.code_index=0;A.nyan_mode=_B;B.addStretch();B.addWidget(G);B.addStretch();B.addWidget(A.pin_checkbox);C.addLayout(B);A.graph=pg.PlotWidget();C.addWidget(A.graph);A.stats=QLabel();A.stats.setAlignment(Qt.AlignmentFlag.AlignCenter);A.stats.setStyleSheet('color: #a7b0d6; font-size: 13px; padding: 8px;');C.addWidget(A.stats);A.setStyleSheet("\n            QDialog { background: #090b18; color: #f4f7fb; font-family: 'Manrope', 'Segoe UI', sans-serif; }\n        ");A.graph.setBackground('#10162a');A.graph.showGrid(x=_A,y=_A,alpha=.25);A.graph.hideButtons();A.graph.setMouseEnabled(_B,_B);A.graph.setMenuEnabled(_B);A.graph.setClipToView(_A);A.graph.setDownsampling(mode='peak');A.graph.setLabel(E,'Speed (KB/s)',color=D);A.graph.setLabel(F,'Time',color=D);A.graph.getAxis(E).setPen(pg.mkPen(I));A.graph.getAxis(F).setPen(pg.mkPen(I));A.graph.getAxis(E).setTextPen(D);A.graph.getAxis(F).setTextPen(D);A.graph.setYRange(0,100);A.download_curve=A.graph.plot(pen=pg.mkPen('#66c7ff',width=3),name=_t);A.upload_curve=A.graph.plot(pen=pg.mkPen(_b,width=3),name=_s);H=A.graph.addLegend();H.setBrush(pg.mkBrush(20,26,46,220));H.setOffset((15,15))
	...
	def generate_nyan_cursor(I):
		...
		A.fillRect(12,9,14,14,QColor('#FFD1DC'));A.fillRect(13,10,12,12,QColor('#FF69B4'));A.fillRect(15,12,2,2,QColor(C));A.fillRect(20,15,2,2,QColor(C));A.fillRect(16,18,2,2,QColor(C));A.fillRect(22,13,9,8,QColor(D));A.fillRect(23,10,2,3,QColor(D));A.fillRect(28,10,2,3,QColor(D));A.fillRect(24,15,2,2,QColor(E));A.fillRect(28,15,2,2,QColor(E));A.fillRect(26,18,2,1,QColor('#FFB6C1'));A.end();return QCursor(B,26,15)
	def toggle_nyan_mode(A):
		A.nyan_mode=not A.nyan_mode
		if A.nyan_mode:A.setWindowTitle('Loqin • Nyan Cat Mode!');A.setCursor(A.generate_nyan_cursor());A.graph.setBackground('#0F051D');A.download_curve.setPen(pg.mkPen('#FF69B4',width=3));A.upload_curve.setPen(pg.mkPen('#00FFFF',width=3));A.stats.setStyleSheet('\n                QLabel{ color:#FFD1DC; font-size:13px; font-weight: bold; }\n            ')
		else:A.setWindowTitle(_A1);A.unsetCursor();A.graph.setBackground('#10162a');A.download_curve.setPen(pg.mkPen('#3da5ff',width=3));A.upload_curve.setPen(pg.mkPen('#2ecc71',width=3));A.stats.setStyleSheet('\n                QLabel{ color:#cccccc; font-size:13px; }\n            ')
...
class SettingsDialog(QDialog):
	...
	def save_settings(A):
		...
		set_auto_start(A.startup_cb.isChecked());A.config[_K]=B;A.config[_U]=A.interval_input.value();ConfigManager.save_config(A.config);ConfigManager.set_password(B,C);QMessageBox.information(A,'Success','Settings saved successfully!');A.accept()
class LoqinTrayApp:
	def __init__(A):A.app=QApplication(sys.argv);A.app.setApplicationName(_F);A.app.setQuitOnLastWindowClosed(_B);A.power_filter=PowerEventFilter(A);A.app.installNativeEventFilter(A.power_filter);A.default_icon=QIcon(resource_path(_J));A.perf_icon=QIcon(resource_path('loqin_logo_performance.png'));A.icon=A.default_icon;A.tray=QSystemTrayIcon();A.tray.setIcon(A.default_icon);A.tray.setVisible(_A);A.tray.showMessage(_F,'Loqin has started! Monitoring your connection in the background.',A.icon,3000);A.config=ConfigManager.load_config();A.last_net_io=psutil.net_io_counters();A.last_time=time.time();A.graph_dialog=_C;A.build_menu();A.wifi_picker=_C;A.waiting_for_wifi_choice=_A;A.selected_wifi_ssid='';A.wifi_connect_thread=_C;A.wifi_startup_thread=_C;A.worker=_C;A.status_action.setText('Status: Looking for your last Wi-Fi...');A.status_action.setIcon(create_status_icon(_D));A.tray.setToolTip('Loqin - Connecting to Wi-Fi');A.force_logout();QTimer.singleShot(1200,A.auto_connect_last_wifi);A.speed_timer=QTimer();A.speed_timer.timeout.connect(A.update_bandwidth_meters);A.speed_timer.start(1000);A.has_checked_for_updates=_B;QTimer.singleShot(1000,A.check_and_auto_connect)
	def build_menu(A):A.menu=QMenu();A.status_action=QAction('Status: Initializing...',A.menu);A.status_action.setIcon(create_status_icon(_D));A.status_action.setEnabled(_A);A.menu.addAction(A.status_action);A.menu.addSeparator();A.speed_action=QAction('Speed: ↓ 0 KB/s  ↑ 0 KB/s',A.menu);A.speed_action.setEnabled(_B);A.menu.addAction(A.speed_action);A.graph_action=QAction(_d,A.menu);A.graph_action.triggered.connect(A.toggle_speed_graph);A.menu.addAction(A.graph_action);A.menu.addSeparator();C=QAction('Connect Now',A.menu);C.triggered.connect(A.trigger_manual_check);A.menu.addAction(C);D=QAction('Choose Wi-Fi',A.menu);D.triggered.connect(A.open_wifi_picker);A.menu.addAction(D);A.pause_action=QAction(_e,A.menu);A.pause_action.triggered.connect(A.toggle_service_pause);A.menu.addAction(A.pause_action);A.perf_action=QAction(_A2,A.menu);A.perf_action.setCheckable(_A);A.perf_action.setChecked(_B);A.perf_action.triggered.connect(A.trigger_performance_mode);A.menu.addAction(A.perf_action);A.menu.addSeparator();A.account_action=QAction('View Account Details',A.menu);A.account_action.setEnabled(_B);A.account_action.triggered.connect(A.show_account_details);A.menu.addAction(A.account_action);A.update_action=QAction(_A3,A.menu);A.update_action.triggered.connect(A.check_for_updates);A.menu.addAction(A.update_action);E=QAction('Configure Settings',A.menu);E.triggered.connect(A.open_settings);A.menu.addAction(E);A.menu.addSeparator();B=A.menu.addMenu('Help');F=QAction('Website',A.menu);F.triggered.connect(lambda:QDesktopServices.openUrl(QUrl('https://loqin-vit.vercel.app/')));B.addAction(F);G=QAction('How to use',A.menu);G.triggered.connect(lambda:QDesktopServices.openUrl(QUrl('https://github.com/notaayushsrivastava/loqin#readme')));B.addAction(G);H=QAction('GitHub Releases',A.menu);H.triggered.connect(lambda:QDesktopServices.openUrl(QUrl('https://github.com/notaayushsrivastava/loqin/releases')));B.addAction(H);I=QAction('Bug Report',A.menu);I.triggered.connect(lambda:QDesktopServices.openUrl(QUrl('https://github.com/notaayushsrivastava/loqin/issues')));B.addAction(I);A.menu.addSeparator();J=QAction('Exit Loqin',A.menu);J.triggered.connect(A.close_app);A.menu.addAction(J);A.tray.setContextMenu(A.menu);A.tray.activated.connect(A.on_tray_icon_activated);A.tray.setToolTip('Loqin PC')
	...
	def toggle_service_pause(A):
		if hasattr(A,_H)and A.worker:
			...
			if B:A.pause_action.setText(_f);A.tray.setToolTip('Loqin - Paused')
			else:A.pause_action.setText(_e);A.tray.setToolTip(_A4)
	def close_app(A):
		...
		if hasattr(A,_A5)and A.perf_thread and A.perf_thread.isRunning():A.perf_thread.quit();A.perf_thread.wait()
		if hasattr(A,_A6)and A.update_checker and A.update_checker.isRunning():A.update_checker.quit();A.update_checker.wait()
		A.app.quit()
	...
	def toggle_speed_graph(A):
		B='Hide Speed Graph'
		if not A.graph_dialog:A.graph_dialog=SpeedGraphDialog();A.graph_dialog.finished.connect(lambda:A.graph_action.setText(_d))
		if A.graph_dialog.isVisible():
			...
			else:A.graph_dialog.hide();A.graph_action.setText(_d)
	...
	def trigger_manual_check(A):
		...
		if A.waiting_for_wifi_choice:A.status_action.setText(_A7);A.status_action.setIcon(create_status_icon(_D));A.tray.setToolTip(_A8);return
		B=_C
		try:
			E=subprocess.check_output([_R,_S,_V,_W],creationflags=134217728).decode(_X,errors=_Y)
			for C in E.split('\n'):
				if _P in C and _P==C.split(_E)[0].strip():
					D=C.split(_E)
	...
	def handle_status(A,message,color_type):
		...
		if B==_A0:
			if hasattr(A,_H)and A.worker:A.worker.is_paused=_A;A.pause_action.setText(_f);A.tray.setToolTip('Loqin - Paused (Missing Credentials)')
	...
	def trigger_performance_mode(A,checked=_B):
		B=checked
		if hasattr(A,_A5)and A.perf_thread.isRunning():return
		if hasattr(A,_H)and A.worker:A.worker.is_paused=_A;A.pause_action.setText(_f);A.tray.setToolTip('Loqin - Paused (Optimizing Network)')
	...
	def handle_perf_status(A,message,color_type):
		C=color_type;B=message;A.status_action.setText(f"Status: {B}");A.status_action.setIcon(create_status_icon(C));A.tray.showMessage(_A2,B,A.icon,4000)
		if B!=_g:
			A.just_optimized=_A
			if hasattr(A,_H)and A.worker:
				A.worker.is_paused=_B;A.pause_action.setText(_e)
				if'OFF'in B:A.tray.setToolTip(_A4)
	...
	def show_account_details(A):
		...
		H=A.config.get(_K);A.account_dialog=AccountDetailsDialog(H,A.current_account_url);A.account_dialog.show();QApplication.processEvents()
		try:
	...
	def check_for_updates(A,silent=_B):
		...
		if hasattr(A,_A6)and A.update_checker and A.update_checker.isRunning():return
		...
		if not B:A.update_checker.no_update_found.connect(A.prompt_no_update);A.update_checker.finished.connect(lambda:A.update_action.setText(_A3));A.update_checker.finished.connect(lambda:A.update_action.setEnabled(_A))
	...
	def install_update(B,exe_path):
		...
		try:
			if sys.platform==_O:os.startfile(A)
	...
	def start_monitoring_timer(A):
		...
		A.timer=QTimer(A.app);A.timer.timeout.connect(A.trigger_manual_check);A.timer.start(A.config.get(_U,10)*1000)
	...
	def save_last_wifi(B,ssid):
		...
		if not A:return
		B.config[_Q]=A;ConfigManager.save_config(B.config)
	def auto_connect_last_wifi(A):
		'Scan for the last Wi-Fi. Use it automatically when it is in range.';B=(A.config.get(_Q)or'').strip()
	...
	def on_startup_scan_finished(A,networks,last_ssid):
		B=last_ssid;C=any(isinstance(A,dict)and A.get(_I,'').strip().lower()==B.lower()for A in networks)
	...
	def on_wifi_connection_success(A,ssid,automatic=_B):
		B=ssid;A.selected_wifi_ssid=B;A.waiting_for_wifi_choice=_B;A.save_last_wifi(B);A.status_action.setText(f"Status: Wi-Fi connected ({B})");A.status_action.setIcon(create_status_icon(_G));A.tray.setToolTip(f"Loqin - Active")
		if A.wifi_picker and A.wifi_picker.isVisible():A.wifi_picker.close()
		A.start_monitoring_timer()
	def on_wifi_connection_failed(A,ssid,error):print(f"Could not connect to {ssid}: {error}");A.waiting_for_wifi_choice=_A;A.status_action.setText(_A7);A.status_action.setIcon(create_status_icon(_D));A.tray.setToolTip(_A8);A.open_wifi_picker()
	def force_logout(B):
	...
	def open_wifi_picker(A):
		...
		else:A.wifi_picker.is_connecting=_B;A.wifi_picker.scan_networks()
		A.wifi_picker.show();A.wifi_picker.raise_();A.wifi_picker.activateWindow()
	def check_and_auto_connect(A):
		'\n        Scans for available Wi-Fi networks and checks if the last connected Wi-Fi is in range.\n        ';C=(A.config.get(_Q)or'').strip();B=get_current_wifi_ssid()
		if B and C and B.lower()==C.lower():print(f"Already connected to '{B}'. Performing login...");A.trigger_manual_check();return
		print('Scanning nearby Wi-Fi networks to locate last connected network...');A.tray.setToolTip('Loqin - Scanning for Wi-Fi...');A.startup_scan_thread=WiFiScanThread();A.startup_scan_thread.networks_found.connect(A._on_auto_connect_scan_finished);A.startup_scan_thread.scan_failed.connect(lambda err:A.open_wifi_picker());A.startup_scan_thread.start()
	def _on_auto_connect_scan_finished(B,networks):
		'Callback handling the results of the initial Wi-Fi range check.';A=(B.config.get(_Q)or'').strip();C=[A.get(_I)for A in networks if A.get(_I)]
		if A and A in C:print(f"Last connected Wi-Fi '{A}' is in range. Auto-connecting...");B.connect_to_wifi(A,automatic=_A)
		else:print(f"Last connected Wi-Fi '{A}' is NOT in range. Opening Wi-Fi Picker...");B.open_wifi_picker()
	def on_wifi_chosen(B,ssid):'Triggered when a Wi-Fi network is selected from the WiFiPickerDialog.';A=ssid;print(f"Wi-Fi selection changed via picker to: {A}");B.save_last_wifi(A);B.connect_to_wifi(A,automatic=_B)
	def connect_and_relogin(A,ssid):'Connects to target SSID, waits for interface handshake, drops session, and logs in.';B=ssid;A.tray.setToolTip(f"Loqin - Connecting to {B}...");A.status_action.setText(f"Connecting to {B}...");A.connect_to_wifi(B,automatic=_B);QTimer.singleShot(4000,A.force_logout_and_relogin)
	def force_logout_and_relogin(A):'Clears existing portal session and initiates fresh captive portal authentication.';print('Resetting portal session and logging in...');A.tray.setToolTip('Loqin - Re-logging in...');A.status_action.setText('Logging in...');A.force_logout();QTimer.singleShot(1000,A.trigger_manual_check)
