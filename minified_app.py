...
_e='perf_thread'
_d='Loqin - Active'
_c='Check for Updates'
_b='Performance Mode'
_a='Show Speed Graph'
_Z='Missing credentials'
_Y='OFFLINE'
_X='Referer'
_W='Grand Total'
_V='https://hostelwifi.vit.ac.in/index.php?a=add&category=4'
_U='Forgot Password?'
...
def create_status_icon(color_type):
	...
	if C==_E:A.setBrush(QColor(46,204,113))
	elif C==_J:A.setBrush(QColor(241,196,15))
...
class PerformanceModeThread(QThread):
	...
	def run(A):
		if A.use_best:A.status_signal.emit('Switching to Performance Mode...',_J)
		else:A.status_signal.emit('Switching to Normal Mode...',_J)
		try:
			F=pywifi.PyWiFi();B=F.interfaces()[0];B.scan();A.sleep(4);G=B.scan_results();D=[A for A in G if'VIT'in(A.ssid or'').upper()]
			if not D:A.status_signal.emit('No VIT networks found in range.',_G);return
			D.sort(key=lambda x:x.signal,reverse=_A);E=D[0];B.disconnect();A.sleep(1);C=pywifi.Profile();C.ssid=E.ssid
			if A.use_best:C.bssid=E.bssid
			C.auth=const.AUTH_ALG_OPEN;C.akm.append(const.AKM_TYPE_NONE);C.cipher=const.CIPHER_TYPE_NONE;B.remove_all_network_profiles();H=B.add_network_profile(C);B.connect(H);A.sleep(4)
			if B.status()==const.IFACE_CONNECTED:
				if A.use_best:A.status_signal.emit(f"Performance Mode ON",_E)
				else:A.status_signal.emit('Normal Mode ON',_E)
			else:A.status_signal.emit('Failed to connect to the network.',_G)
		except Exception as I:A.status_signal.emit(f"Wi-Fi Error: {str(I)}",_G)
...
class AccountDetailsDialog(QDialog):
	...
	def populate_table(A,rows_data,grand_total_data):
		...
		if F:
			C=A.table.rowCount();A.table.insertRow(C);E=QTableWidgetItem(_W);E.setForeground(QColor(_N));E.setTextAlignment(Qt.AlignmentFlag.AlignCenter);A.table.setItem(C,0,E)
...
class NetworkWorker(QThread):
	...
	def check_network_state(B):
		...
		except requests.exceptions.RequestException:return _Y
	def run(A):
		while A.is_running:
			if A.is_paused:A.sleep(1);continue
			B=A.config.get(_H);C=ConfigManager.get_password(B)
			if not B or not C:A.status_signal.emit(_Z,_G);return
			D=A.check_network_state()
			if D==_O:A.status_signal.emit('Connected',_E);return
			elif D==_Y:A.status_signal.emit('Waiting for Wi-Fi...',_J);return
			A.status_signal.emit('Portal detected. Authenticating...',_J);A.login(B,C);return
	def toggle_pause(A):A.is_paused=not A.is_paused;return A.is_paused
	def login(A,username,password):
		B='http://phc.prontonetworks.com';D=f"{B}/cgi-bin/authlogin?URI=http://example.com";E={'userId':username,_I:password,'serviceName':'ProntoAuthentication','URI':'http://example.com'};F={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64)','Content-Type':'application/x-www-form-urlencoded','Origin':B,_X:f"{B}/cgi-bin/authlogin?URI=http://www.msftconnecttest.com/redirect"}
		try:
			G=requests.post(D,data=E,headers=F,timeout=5)
			if A.check_network_state()==_O:
				A.status_signal.emit('Logged in successfully!',_E);H=G.text;C=re.search('href="(http://([0-9\\.]+)/registration/Main\\.jsp\\?sessionId=[^"]+)"',H)
...
class LoqinTrayApp:
	...
	def toggle_service_pause(A):
		if hasattr(A,_F)and A.worker:
			...
			else:A.pause_action.setText(_P);A.tray.setToolTip(_d)
	def close_app(A):
		try:requests.get('http://phc.prontonetworks.com/cgi-bin/authlogout/',timeout=2)
		except Exception:pass
		if hasattr(A,_F)and A.worker and A.worker.isRunning():A.worker.is_running=_B;A.worker.quit();A.worker.wait()
		if hasattr(A,_e)and A.perf_thread and A.perf_thread.isRunning():A.perf_thread.quit();A.perf_thread.wait()
		if hasattr(A,_f)and A.update_checker and A.update_checker.isRunning():A.update_checker.quit();A.update_checker.wait()
		A.app.quit()
	...
	def trigger_performance_mode(A,checked=_B):
		B=checked
		if hasattr(A,_e)and A.perf_thread.isRunning():return
		if hasattr(A,_F)and A.worker:A.worker.is_paused=_A;A.pause_action.setText(_Q);A.tray.setToolTip('Loqin - Paused (Optimizing Network)')
	...
	def handle_perf_status(A,message,color_type):
		C=message;B=color_type;A.status_action.setText(f"Status: {C}");A.status_action.setIcon(create_status_icon(B));A.tray.showMessage(_b,C,A.icon,4000)
		if B==_E or B==_G:
			if hasattr(A,_F)and A.worker:A.worker.is_paused=_B;A.pause_action.setText(_P);A.tray.setToolTip(_d)
			if B==_E:QTimer.singleShot(1000,A.trigger_manual_check)
	...
	def show_account_details(A):
		...
		try:
			...
			F=[];G=D.find(string=lambda text:text and _W in text)
	...
	def check_for_updates(A):
		if hasattr(A,_f)and A.update_checker and A.update_checker.isRunning():return
		A.update_action.setText('Checking for updates...');A.update_action.setEnabled(_B);A.update_checker=UpdateChecker();A.update_checker.update_found.connect(A.prompt_update);A.update_checker.finished.connect(lambda:A.update_action.setText(_c));A.update_checker.finished.connect(lambda:A.update_action.setEnabled(_A));A.update_checker.start()
