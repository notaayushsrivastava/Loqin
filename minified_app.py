...
_j='perf_thread'
_i='Loqin - Active'
_h='Check for Updates'
_g='Performance Mode'
_f='#FF69B4'
_e='#3da5ff'
_d='#171A22'
_c='Loqin • Live Network Monitor'
_b='Missing credentials'
_a='OFFLINE'
_Z='Referer'
_Y='Grand Total'
_X='https://hostelwifi.vit.ac.in/index.php?a=add&category=4'
_W='Forgot Password?'
_V='\n            QPushButton {\n                background: #1E222D; \n                color: #BBBBBB; \n                border: 1px solid #2C313E; \n                border-radius: 4px; \n                font-size: 14px;\n            }\n            QPushButton:checked {\n                background: #3da5ff; \n                color: #171A22; \n                border: 1px solid #3da5ff;\n            }\n            QPushButton:hover {\n                border: 1px solid #3da5ff;\n            }\n        '
_U='Download'
_T='Upload'
_S='Optimizing Network...'
_R='Resume Loqin'
_Q='Pause Loqin'
_P='Show Speed Graph'
_O='ONLINE'
...
class PerformanceModeThread(QThread):
	...
	def run(A):
		L='Performance Mode ON';K='BSSID';D=':';M=_S if A.use_best else'Reverting Network...';A.status_signal.emit(M,_G)
		try:
...
class AccountDetailsDialog(QDialog):
	...
	def populate_table(A,rows_data,grand_total_data):
		...
		if F:
			C=A.table.rowCount();A.table.insertRow(C);E=QTableWidgetItem(_Y);E.setForeground(QColor(_M));E.setTextAlignment(Qt.AlignmentFlag.AlignCenter);A.table.setItem(C,0,E)
			for(J,D)in enumerate(F):B=QTableWidgetItem(D);B.setForeground(QColor(_M));B.setTextAlignment(Qt.AlignmentFlag.AlignCenter);A.table.setItem(C,J+3,B)
...
class ConfigManager:
	...
	@staticmethod
	def load_config():
		A={_H:'',_N:10,'auto_connect':_B};ConfigManager.ensure_dir_exists();B=_A
...
class NetworkWorker(QThread):
	...
	def check_network_state(B):
		...
		except requests.exceptions.RequestException:return _a
	def run(A):
		while A.is_running:
			if A.is_paused:A.sleep(1);continue
			B=A.config.get(_H);C=ConfigManager.get_password(B)
			if not B or not C:A.status_signal.emit(_b,_J);return
			D=A.check_network_state()
			if D==_O:A.status_signal.emit('Connected',_F);return
			elif D==_a:A.status_signal.emit('Waiting for Wi-Fi...',_G);return
	...
	def login(A,username,password):
		B='http://phc.prontonetworks.com';D=f"{B}/cgi-bin/authlogin?URI=http://example.com";E={'userId':username,_I:password,'serviceName':'ProntoAuthentication','URI':'http://example.com'};F={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64)','Content-Type':'application/x-www-form-urlencoded','Origin':B,_Z:f"{B}/cgi-bin/authlogin?URI=http://www.msftconnecttest.com/redirect"}
		try:
...
class SpeedGraphDialog(QDialog):
	def __init__(A,parent=_C):I='#777';F='bottom';E='left';D='#BBBBBB';super().__init__(parent);A.setWindowTitle(_c);A.setWindowIcon(QIcon(resource_path(_K)));A.resize(720,420);A.download_history=[0]*60;A.upload_history=[0]*60;C=QVBoxLayout(A);C.setContentsMargins(15,15,15,15);B=QHBoxLayout();G=QLabel('Real-Time Network Usage');G.setStyleSheet('\n            QLabel{\n                color:white;\n                font-size:18px;\n                font-weight:600;\n            }\n        ');A.pin_checkbox=QCheckBox('Always on Top');A.pin_checkbox.setCursor(Qt.CursorShape.PointingHandCursor);A.pin_checkbox.setStyleSheet('\n            QCheckBox {\n                color: #BBBBBB;\n                font-size: 13px;\n                font-weight: 500;\n            }\n            QCheckBox::indicator {\n                width: 14px;\n                height: 14px;\n                border: 1px solid #777777;\n                border-radius: 3px;\n                background: #171A22;\n            }\n            QCheckBox::indicator:checked {\n                background: #3da5ff;\n                border: 1px solid #3da5ff;\n            }\n            QCheckBox:hover {\n                color: #FFFFFF;\n            }\n        ');A.pin_checkbox.toggled.connect(A.toggle_always_on_top);A.secret_code='nyan';A.code_index=0;A.nyan_mode=_A;B.addStretch();B.addWidget(G);B.addStretch();B.addWidget(A.pin_checkbox);C.addLayout(B);A.graph=pg.PlotWidget();C.addWidget(A.graph);A.stats=QLabel();A.stats.setAlignment(Qt.AlignmentFlag.AlignCenter);A.stats.setStyleSheet('\n            QLabel{\n                color:#cccccc;\n                font-size:13px;\n            }\n        ');C.addWidget(A.stats);A.setStyleSheet('\n            QDialog{\n                background:#171A22;\n            }\n        ');A.graph.setBackground(_d);A.graph.showGrid(x=_B,y=_B,alpha=.25);A.graph.hideButtons();A.graph.setMouseEnabled(_A,_A);A.graph.setMenuEnabled(_A);A.graph.setClipToView(_B);A.graph.setDownsampling(mode='peak');A.graph.setLabel(E,'Speed (KB/s)',color=D);A.graph.setLabel(F,'Time',color=D);A.graph.getAxis(E).setPen(pg.mkPen(I));A.graph.getAxis(F).setPen(pg.mkPen(I));A.graph.getAxis(E).setTextPen(D);A.graph.getAxis(F).setTextPen(D);A.graph.setYRange(0,100);A.download_curve=A.graph.plot(pen=pg.mkPen(_e,width=3),name=_U);A.upload_curve=A.graph.plot(pen=pg.mkPen(_M,width=3),name=_T);H=A.graph.addLegend();H.setBrush(pg.mkBrush(30,30,30,200));H.setOffset((15,15))
	...
	def generate_nyan_cursor(I):
		...
		A.fillRect(12,9,14,14,QColor('#FFD1DC'));A.fillRect(13,10,12,12,QColor(_f));A.fillRect(15,12,2,2,QColor(C));A.fillRect(20,15,2,2,QColor(C));A.fillRect(16,18,2,2,QColor(C));A.fillRect(22,13,9,8,QColor(D));A.fillRect(23,10,2,3,QColor(D));A.fillRect(28,10,2,3,QColor(D));A.fillRect(24,15,2,2,QColor(E));A.fillRect(28,15,2,2,QColor(E));A.fillRect(26,18,2,1,QColor('#FFB6C1'));A.end();return QCursor(B,26,15)
	def toggle_nyan_mode(A):
		A.nyan_mode=not A.nyan_mode
		if A.nyan_mode:A.setWindowTitle('Loqin • Nyan Cat Mode!');A.setCursor(A.generate_nyan_cursor());A.graph.setBackground('#0F051D');A.download_curve.setPen(pg.mkPen(_f,width=3));A.upload_curve.setPen(pg.mkPen('#00FFFF',width=3));A.stats.setStyleSheet('\n                QLabel{ color:#FFD1DC; font-size:13px; font-weight: bold; }\n            ')
		else:A.setWindowTitle(_c);A.unsetCursor();A.graph.setBackground(_d);A.download_curve.setPen(pg.mkPen(_e,width=3));A.upload_curve.setPen(pg.mkPen(_M,width=3));A.stats.setStyleSheet('\n                QLabel{ color:#cccccc; font-size:13px; }\n            ')
...
class SettingsDialog(QDialog):
	...
	def save_settings(A):
		...
		set_auto_start(A.startup_cb.isChecked());A.config[_H]=B;A.config[_N]=A.interval_input.value();ConfigManager.save_config(A.config);ConfigManager.set_password(B,C);QMessageBox.information(A,'Success','Settings saved successfully!');A.accept()
class LoqinTrayApp:
	def __init__(A):A.app=QApplication(sys.argv);A.app.setApplicationName(_D);A.app.setQuitOnLastWindowClosed(_A);A.power_filter=PowerEventFilter(A);A.app.installNativeEventFilter(A.power_filter);A.default_icon=QIcon(resource_path(_K));A.perf_icon=QIcon(resource_path('loqin_logo_performance.png'));A.icon=A.default_icon;A.tray=QSystemTrayIcon();A.tray.setIcon(A.default_icon);A.tray.setVisible(_B);A.tray.showMessage(_D,'Loqin has started! Monitoring your connection in the background.',A.icon,3000);A.config=ConfigManager.load_config();A.last_net_io=psutil.net_io_counters();A.last_time=time.time();A.graph_dialog=_C;A.build_menu();A.worker=_C;A.start_monitoring_timer();A.force_logout();A.speed_timer=QTimer();A.speed_timer.timeout.connect(A.update_bandwidth_meters);A.speed_timer.start(1000);A.has_checked_for_updates=_A
	def build_menu(A):A.menu=QMenu();A.status_action=QAction('Status: Initializing...',A.menu);A.status_action.setIcon(create_status_icon(_G));A.status_action.setEnabled(_B);A.menu.addAction(A.status_action);A.menu.addSeparator();A.speed_action=QAction('Speed: ↓ 0 KB/s  ↑ 0 KB/s',A.menu);A.speed_action.setEnabled(_A);A.menu.addAction(A.speed_action);A.graph_action=QAction(_P,A.menu);A.graph_action.triggered.connect(A.toggle_speed_graph);A.menu.addAction(A.graph_action);A.menu.addSeparator();C=QAction('Connect Now',A.menu);C.triggered.connect(A.trigger_manual_check);A.menu.addAction(C);A.pause_action=QAction(_Q,A.menu);A.pause_action.triggered.connect(A.toggle_service_pause);A.menu.addAction(A.pause_action);A.perf_action=QAction(_g,A.menu);A.perf_action.setCheckable(_B);A.perf_action.setChecked(_A);A.perf_action.triggered.connect(A.trigger_performance_mode);A.menu.addAction(A.perf_action);A.menu.addSeparator();A.account_action=QAction('View Account Details',A.menu);A.account_action.setEnabled(_A);A.account_action.triggered.connect(A.show_account_details);A.menu.addAction(A.account_action);A.update_action=QAction(_h,A.menu);A.update_action.triggered.connect(A.check_for_updates);A.menu.addAction(A.update_action);D=QAction('Configure Settings',A.menu);D.triggered.connect(A.open_settings);A.menu.addAction(D);A.menu.addSeparator();B=A.menu.addMenu('Help');E=QAction('How to use',A.menu);E.triggered.connect(lambda:QDesktopServices.openUrl(QUrl('https://github.com/notaayushsrivastava/loqin#readme')));B.addAction(E);F=QAction('GitHub Releases',A.menu);F.triggered.connect(lambda:QDesktopServices.openUrl(QUrl('https://github.com/notaayushsrivastava/loqin/releases')));B.addAction(F);G=QAction('Project Info',A.menu);G.triggered.connect(lambda:QDesktopServices.openUrl(QUrl('https://github.com/notaayushsrivastava/loqin')));B.addAction(G);A.menu.addSeparator();H=QAction('Exit Loqin',A.menu);H.triggered.connect(A.close_app);A.menu.addAction(H);A.tray.setContextMenu(A.menu);A.tray.activated.connect(A.on_tray_icon_activated);A.tray.setToolTip('Loqin PC')
	...
	def toggle_service_pause(A):
		if hasattr(A,_E)and A.worker:
			...
			if B:A.pause_action.setText(_R);A.tray.setToolTip('Loqin - Paused')
			else:A.pause_action.setText(_Q);A.tray.setToolTip(_i)
	def close_app(A):
		try:requests.get('http://phc.prontonetworks.com/cgi-bin/authlogout/',timeout=2)
		except Exception:pass
		if hasattr(A,_E)and A.worker and A.worker.isRunning():A.worker.is_running=_A;A.worker.quit();A.worker.wait()
		if hasattr(A,_j)and A.perf_thread and A.perf_thread.isRunning():A.perf_thread.quit();A.perf_thread.wait()
		if hasattr(A,_k)and A.update_checker and A.update_checker.isRunning():A.update_checker.quit();A.update_checker.wait()
		A.app.quit()
	...
	def toggle_speed_graph(A):
		B='Hide Speed Graph'
		if not A.graph_dialog:A.graph_dialog=SpeedGraphDialog();A.graph_dialog.finished.connect(lambda:A.graph_action.setText(_P))
		if A.graph_dialog.isVisible():
			...
			else:A.graph_dialog.hide();A.graph_action.setText(_P)
	...
	def handle_status(A,message,color_type):
		...
		if B==_b:
			if hasattr(A,_E)and A.worker:A.worker.is_paused=_B;A.pause_action.setText(_R);A.tray.setToolTip('Loqin - Paused (Missing Credentials)')
			QTimer.singleShot(100,A.open_settings);return
		if C==_F:
			if'successfully'in B:A.tray.showMessage(_D,B,A.icon,3000)
			if not getattr(A,'has_checked_for_updates',_A):A.check_for_updates(_B);A.has_checked_for_updates=_B
		elif C==_J:A.tray.showMessage(_D,B,A.icon,3000)
	def trigger_performance_mode(A,checked=_A):
		B=checked
		if hasattr(A,_j)and A.perf_thread.isRunning():return
		if hasattr(A,_E)and A.worker:A.worker.is_paused=_B;A.pause_action.setText(_R);A.tray.setToolTip('Loqin - Paused (Optimizing Network)')
	...
	def handle_perf_status(A,message,color_type):
		C=color_type;B=message;A.status_action.setText(f"Status: {B}");A.status_action.setIcon(create_status_icon(C));A.tray.showMessage(_g,B,A.icon,4000)
		if B!=_S:
			if hasattr(A,_E)and A.worker:
				A.worker.is_paused=_A;A.pause_action.setText(_Q)
				if'OFF'in B:A.tray.setToolTip(_i)
	...
	def show_account_details(A):
		...
		try:
			...
			F=[];G=D.find(string=lambda text:text and _Y in text)
	...
	def check_for_updates(A,silent=_A):
		...
		if hasattr(A,_k)and A.update_checker and A.update_checker.isRunning():return
		...
		if not B:A.update_checker.no_update_found.connect(A.prompt_no_update);A.update_checker.finished.connect(lambda:A.update_action.setText(_h));A.update_checker.finished.connect(lambda:A.update_action.setEnabled(_B))
	...
	def start_monitoring_timer(A):
		...
		A.timer=QTimer();A.timer.timeout.connect(A.trigger_manual_check);A.timer.start(A.config.get(_N,10)*1000);A.trigger_manual_check()
	def force_logout(B):
